...
let DirectChildHasId sInfo id =
     sInfo.Children |> Seq.exists (fun child -> child.StatusId() = id) //filter ids not in Children

let AnyChildHasId sInfo id =
    let flattened = sInfo.Children |> Flatten
    flattened |> Seq.exists (fun child -> child.StatusId() = id) //filter ids not in Children or deeper in children